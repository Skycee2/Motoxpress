...
export class CartPage implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
      ...
      if(this.location?.id && this.location?.id != '') {
        ...
        if(result) {
          this.global.errorToast(
            'Su ubicación está demasiado lejos del restaurante en el carrito, busque amablemente desde algún otro restaurante cercano.',
            5000);
    ...
    this.cartSub = this.cartService.cart.subscribe(cart => {
      ...
      console.log('modelo de página de ...: ', this.model);
    });
  }
  ...
  async makePayment() {
    try {
      ...
      this.model = {} as Cart;
      this.global.successToast('Su pedido se realiza con éxito');
